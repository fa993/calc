...
use crate::{CalcFunctionData, CalcNode};
...
pub fn assemble_map_veri_nand() -> HashMap<String, EvalFunction> {
    ...
    ur.insert(
        "multiply".to_string(),
        Box::new(|t, counter: Arc<AtomicUsize>| {
            bound_check(&t, 2);
    ...
    ur.insert(
        "add".to_string(),
        Box::new(|t: Vec<_>, counter: Arc<AtomicUsize>| {
            bound_check(&t, 2);
